            ...
            for ym in months:
                ...
                for attempt in range(max_retries + 1):
                    ...
                    if df_raw_trade is not None:
                        ...
                        trade_ok = True
                        break
                    time.sleep(current_interval * (attempt + 2))  # 지수 백오프
        ...
        if df_trade_list:
            ...
            if failed_list:
                st.warning(
                    ...
                    f"실패한 {len(failed_list)}개 구는 사이드바 [재시도] 버튼으로 복구 가능합니다."
                )
            else:
