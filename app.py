...
st.caption("실거래가 정밀 분석 + 🌟 [NEW] AI 실시간 구글 검색(웹 브라우징) 연동 + 맞춤형 청약 컨설팅")
st.markdown("---")
...
with tab2:
    try:
        ...
        if not df_sheet.empty and '매매가(억)' in df_sheet.columns:
            ...
            st.info("위 리스트에서 기준이 될 관심 아파트를 선택해 AI와 상담해보세요. 다른 아파트와의 비교를 물어보면 AI가 실시간 검색을 통해 알아서 대답해 줍니다!")
            ...
            selected_key = st.selectbox("상담할 기준 매물 검색", apt_list, index=None, placeholder="예: 강남구 은마...")
            ...
            if selected_key:
                ...
                if st.button("🚀 매매 심층 분석 시작", type="primary"):
                    ...
                    [기준 매물] {target['아파트명']} ({target['지역']}), {target.get('건축년도','-')}년 건축, {target.get('층','-')}층, {target['평형']}평
                    ...
                    먼저 이 기준 매물의 가격 적정성, '층'과 '건축년도'를 고려한 투자/실거주 적합성, 자금 여력을 종합 분석해줘.
                    """
                    st.session_state['context_prompt_tab2'] = system_prompt
                    
                    with st.spinner("AI가 입체적으로 분석 중입니다..."):
                        try:
                            # [핵심] tools='google_search_retrieval' 파라미터 추가로 웹 검색 기능 활성화!
            ...
            else: st.info("👆 분석할 기준 매물을 선택해주세요.")
    ...
    if 'apply_data' in st.session_state:
        ...
        if df_apply is not None and not df_apply.empty:
            ...
            if st.button("✨ 내 조건에 맞는 단지 추천 및 자문 시작", type="primary"):
                ...
                🔥중요 지시사항🔥
                사용자가 위 리스트에 없는 다른 예정된 청약 단지나 해당 지역의 주변 시세(안전마진)에 대해 물어보면, 반드시 너에게 내장된 '구글 실시간 검색(Google Search)' 기능을 적극적으로 활용해서 최신 분양가, 주변 시세, 청약 호재 등을 찾아내서 대답해줘.
                
                위 정보를 바탕으로 아래 리포트를 작성하고, 이후 사용자의 질문에 답해줘:
                1. 맞춤형 전략 추천: 사용자의 신혼/다자녀/무주택 조건에 비추어 볼 때 가점제, 추첨제, 혹은 특정 특별공급 중 어느 전형으로 넣는 것이 당첨 확률이 가장 높은지 구체적으로 조언해줘.
            ...
            if prompt := st.chat_input("청약 전략, 대출, 주변 시세 등에 대해 자유롭게 질문하세요!"):
                with st.chat_message("user"): st.markdown(prompt)
